...
describe "The parser" do
  ...
  it "should parse 1d6 Fire to a typed roll" do
    subject = Dice.parse('1d6 Fire')
    subject.should be_kind_of(Dice::TypedSet)
    subject.to_s.should == '1d6 Fire'
    subject
  end
...
describe Dice::Result do
  def self.describe_dice(dice, total, roll_details, roll_options={}, &block)
    describe "result for #{dice}" do
      ...
      it { should be_kind_of(Dice::Result) }
      its(:total_string) { should == total.to_s }
      its(:roll_details) { should == roll_details }
  ...
  context "with an expected result set" do
    ...
    describe_dice "5d6", 15, "[5d6: 1,2,3,4,5]"
    describe_dice "5d5", 15, "[5d5: 1,2,3,4,5]"
    describe_dice "(1d6+4d6)", 15, "[1d6: 1] [4d6: 2,3,4,5]"
